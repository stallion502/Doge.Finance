...
//  BrokerDomainCore
//
//  Created by Rishat Yakushev on 13/02/2019.
//  Copyright © 2019 AkBars. All rights reserved.
//
import UIKit
...
public extension Date {
    ...
    static func date(with additionalDays: Int) -> Date {
        var calendar = Calendar.current
        calendar.timeZone = .utc
        let startOfDay = calendar.startOfDay(for: Date())
        var components = DateComponents()
        components.day    = additionalDays
        return (calendar as NSCalendar).date(byAdding: components, to: startOfDay, options: NSCalendar.Options()) ?? .init()
    }
    ...
    func string(_ format: Formats, timeZone: TimeZone = .current) -> String {
        if case .relative = format {
            return self.relative(.minute)
        }
...
public extension Date {
    ...
    private func relativeMinute(relativeType: RelativeDateFormat, short: Bool = false) -> String {
        switch relativeType {
        case .today:
            return "Сегодня в \(DateFormatterBuilder.HHmm.string(from: self))"
        case .yesterday:
            return "Вчера в \(DateFormatterBuilder.HHmm.string(from: self))"
        case .thisYear:
            return "\(DateFormatterBuilder.dateFormatter(short ? .dMMM : .dMMMM).string(from: self)) в \(DateFormatterBuilder.dateFormatter(.HHmm).string(from: self))"
        case .yearAgo:
            return "\(DateFormatterBuilder.dateFormatter(short ? .dMMMyyyy : .dMMMMyyyy).string(from: self)) в \(DateFormatterBuilder.dateFormatter(.HHmm).string(from: self))"
        }
    }
    ...
    private func relativeQuoteTime(relativeType: RelativeDateFormat) -> String {
        ...
        default:
            return "\(DateFormatterBuilder.dateFormatter(.dMMM).string(from: self)) в \(DateFormatterBuilder.dateFormatter(.HHmmss).string(from: self))"
        }
    }
}
